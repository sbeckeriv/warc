use nom::{IResult,not_line_ending, line_ending, space, alphanumeric, multispace, digit, Needed, Err};
use nom::IResult::*;
// Parser definition
use std::str;
...
fn token_colon(input: &[u8]) -> IResult<&[u8], &[u8]> {
    for (idx, chr) in input.iter().enumerate() {
        match *chr {
            33...126 => continue,
            _ => return IResult::Done(&input[idx..], &input[..idx]),
...
dbg!(chain!(
        ...
        tag!(":")                   ~
        space?                      ~
        value: map_res!(token_colon, str::from_utf8)~
        tag!("\r")?                 ~
        tag!("\n")                  ,
        || {(name, value)}
        )
    ...
    match warc_header(input) {
        IResult::Done(mut i,tuple_vec) => {
            ...
            match h.get(&"Content-Length"){
                ...
                _ => println!("shit no content-length")
            }
            IResult::Done(i, h)
        },
